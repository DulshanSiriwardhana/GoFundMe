...
export default function Layout({ children }: { children: ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-[#f0fdf4] text-[#014d40] font-serif selection:bg-emerald-100 selection:text-emerald-900">
                        ...
                        <div className="hidden md:flex items-center gap-12">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.path}
                                    to={link.path}
                                    className={`flex items-center gap-2 text-sm font-bold transition-all relative py-1 ${location.pathname === link.path
                                        ...
                                        : "text-emerald-900/60 hover:text-emerald-600"
                                        }`}
                                >
                                    {link.name}
                                </Link>
                        ...
                        <div className="hidden md:flex items-center gap-6">
                            {account ? (
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full text-sm font-black shadow-lg shadow-emerald-600/20 transition-all active:scale-95"
                                >
            ...
            <main className="pt-36 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                {children}
            </main>

            <footer className="bg-emerald-950 text-emerald-100/40 border-t border-emerald-900 py-20">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-12">
                        <div className="flex items-center gap-3 group cursor-pointer">
                            <div className="w-10 h-10 rounded-xl overflow-hidden shadow-lg shadow-emerald-950/50 group-hover:scale-105 transition-all">
                                <img src="/favicon.png" alt="GoFundChain Logo" className="w-full h-full object-cover" />
                            </div>
                        ...
                        <div className="text-xs font-bold uppercase tracking-widest text-emerald-100/30">
                            <p>&copy; 2026 GoFundChain Protocol. All Rights Reserved.</p>
                        </div>
