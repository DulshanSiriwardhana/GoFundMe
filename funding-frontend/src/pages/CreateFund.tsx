  ...
  return (
    <div className="max-w-4xl mx-auto py-10 md:py-20 px-4">
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-4 pt-6">
            {[
              ...
              <div key={i} className="flex gap-4">
                <div className="p-2.5 bg-white border border-emerald-100 rounded-xl text-emerald-500 shadow-sm h-fit">
                  <f.icon className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="font-black text-emerald-950 text-sm leading-tight">{f.label}</h4>
                  <p className="text-xs text-emerald-900/40 font-medium">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card className="p-8 md:p-10 border border-emerald-50 rounded-2xl bg-white shadow-xl relative overflow-hidden">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full bg-emerald-950 hover:bg-black text-white py-5 rounded-xl font-black text-lg shadow-xl shadow-emerald-950/10 active:scale-[0.98] transition-all flex items-center justify-center gap-3 ${loading ? 'cursor-wait opacity-80' : ''}`}
            >
              {loading ? (
                <>
          ...
          <div className="mt-8 p-4 bg-amber-50/50 border border-amber-100 rounded-xl flex items-start gap-4">
            <AlertCircle className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
            <p className="text-[10px] text-amber-900/60 font-bold uppercase tracking-widest leading-loose">
